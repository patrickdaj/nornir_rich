...
@click.option(
    "-g", "--groups", help="Filter hosts using groups\n" + "Example: 'group_a,group_b'"
)
@click.option(
    "-h", "--hosts", help="Filter hosts using names\n" 'Example: "host_a,host_b"'
)
@click.option("-c", "--config", default="config.yaml", help="Nonrnir config file path")
@click.option("-o", "--options", help="Inventory options\n" + 'Example: "NB_KEY=mykey"')
@click.option(
    "-p",
    "--python",
...
def get_filtered(nr: Nornir, params: Dict[str, Any]) -> Nornir:
    ...
    if 'kwargs' in params and params['kwargs']:
...
@cli.command()
@click.option("-p/", "--passwords", is_flag=True, help="Display passwords")
@click.pass_context
...
@click.option(
    "-a", "--args", help="Arguments required by module" + "Example: 'x=1,z=244'"
)
@click.pass_context
def adhoc(ctx: click.Context, module: str, args: str):
    ...
    filtered = get_filtered(nr, ctx.parent.params)
    filtered.run(
        task=task, **{k: v for k, v in [a.split("=") for a in args.split(",")]}
    )


@cli.command()
@click.option("-r", "--run-file", help="YAML file of tasks to execute")
@click.pass_context
def run(ctx: click.Context, run_file: str):
    ...
    for sub_plan in plan:
        ...
        for task in sub_plan["tasks"]:
            task_function = get_task_function(task["task"])
